...
public:
  ...
  void run( const double offMaxAmpsThreshold, const double onMinAmpsThreshold, uint32_t onMillis = DEFAULT_CHECK_ONMILLIS, uint32_t offMillis = DEFAULT_CHECK_OFFMILLIS );

  // measure amps and compare against threshold for the current relay state
  bool check( const double offMaxAmpsThreshold, const double onMinAmpsThreshold );
};
